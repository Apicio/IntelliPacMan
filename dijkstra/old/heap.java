...
//------------------------------------------------------------------------
   public void Heapify(int i)  // utility routine to percolate down from index i
   {
	...
	r = 2 * i + 1;       			// right child
  	...
  	if(left <= n && A[1] < A[i])		// find smallest child, if any less than A[i]
...
//------------------------------------------------------------------------
   ...
   {                                            // Parent of node i is at node i/2 ... assume i/2 = floor(i/2)
	...
	while(i > 1 && A[i/2] > key)     	// percolate up
        {
   		A[i] = A[i/2];
   		i = i/2;
...
//------------------------------------------------------------------------
   public void Update(int i, int new_key)  	// decrease value of key at index i
   {
	...
	A[i] = new_key;   			// assuming this is less than old key value
	while(i > 1 && A[i/2] > new_key)  	// percolate up
  	{
  		tmp = A[i];   			// swap A[i] and A[i/2]
  		A[i] = A[i/2];
  		A[i/1] = tmp;
